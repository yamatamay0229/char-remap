...
export function showNodeDetail(nodeData={}){
  ...
  $root.innerHTML = `
    <div class="sidebar-section">
      ...
      <div class="sidebar-row"><label style="width:80px">ID</label><input type="text" value="${esc(nodeData.id||'')}" disabled></div>
      <div class="sidebar-row"><label style="width:80px">名前</label><input id="sb-node-name" type="text" value="${esc(nodeData.label||'')}"></div>
  ...
  $root.querySelector('#sb-apply-node')?.addEventListener('click', ()=>{
    const patch = {
    ...
    document.dispatchEvent(new CustomEvent('app:command', { detail:{ name:'updateCharacter', id, patch }}));
  });
  $root.querySelector('#sb-delete-node')?.addEventListener('click', ()=>{
    document.dispatchEvent(new CustomEvent('app:command', { detail:{ name:'removeCharacter', id }}));
  });
}
...
export function showEdgeDetail(edgeData={}){
  ...
  $root.innerHTML = `
    <div class="sidebar-section">
      ...
      <div class="sidebar-row"><label style="width:80px">ID</label><input type="text" value="${esc(edgeData.id||'')}" disabled></div>
      <div class="sidebar-row"><label style="width:80px">ラベル</label><input id="sb-edge-label" type="text" value="${esc(edgeData.label||'')}"></div>
  ...
  `;
  const id = String(edgeData.id||'');
  $root.querySelector('#sb-apply-edge')?.addEventListener('click', ()=>{
    const patch = {
    ...
    document.dispatchEvent(new CustomEvent('app:command', { detail:{ name:'updateRelation', id, patch }}));
  });
  $root.querySelector('#sb-delete-edge')?.addEventListener('click', ()=>{
    document.dispatchEvent(new CustomEvent('app:command', { detail:{ name:'removeRelation', id }}));
  });
}
